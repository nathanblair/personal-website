...
<label for="dark-toggle" class="mx-2">
	...
	/>
	<span
		class="border-surface-400-600 flex h-6 w-10 cursor-pointer items-center rounded-full p-0.5 hover:brightness-90 dark:hover:brightness-110 {dark_mode.value
	...
	>
		<span
			class="!translate-x-{dark_mode.value
				? '4'
				: '0'} bg-surface-50 rounded-full p-0.5 transition duration-200 ease-in-out"
		>
			{#if dark_mode.value}
				<Moon class="size-4" />
			{:else}
